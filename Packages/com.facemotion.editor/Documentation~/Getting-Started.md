...
3. `プロジェクト` の **新規プロジェクト** で FaceMotion Project asset を作成し、`アニメーション` の **+ 新規アニメーション** で Animation を作成します。Animation を作成したら `アニメーション名` を編集できます（VRChat の menu label にそのまま使われます）。
4. avatar を選択した状態で `トラック` の **+ ブレンドシェイプ** または **+ トランスフォーム** を選びます。BlendShape browser のカテゴリ、検索、conflict 表示を確認し、必要なら `詳細な手動バインディング` を使います。BlendShape Track 追加時は現在の BlendShape 値を 0 秒の初期 key にします。
